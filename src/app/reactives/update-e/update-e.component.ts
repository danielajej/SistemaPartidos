...
export class UpdateEComponent implements OnInit {
  ...
  ObtenerRegistro(id: any) {
    this.conexion.Post('futbolistas', 'GetIdE', { 'id': id }).subscribe((dato: any) => {
    ...
    guardar() {
    this.conexion.Post('futbolistas', 'UpdateE', this.Formulario.value).subscribe((dato: any) => {
      console.log(dato);
      if (dato['estatus']) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {this.conexion.Post('futbolistas', 'DeleteE', { id: id }).subscribe((dato: any) => {
        console.log(dato);
        if (dato['estatus']) {
          this.router.navigate(['reactive/equipos']);
        Swal.fire(
